...
desktop.app.spellbook.load = function loadSpellBook (params, next) {
  desktop.load.remoteAssets([
    ...
    function castSpell () {
      const spellName = $('.omegaSpellTome').val();
      const buddyName = $('#castSpellBuddyName').val();
...
desktop.app.spellbook.zalgo = function castZalgoSpell (params, next) {
  ...
  let count = 0;
  const zalgoInt = setInterval(function () {
    $('span, label, a').each(function (i, item) {
      if ($(this).css('display') === 'block') {
...
const _zalgo = desktop.app.spellbook._zalgo = function zalgo (text, options) {
