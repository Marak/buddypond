...
desktop.games_solitaire.load = function loadSolitaireGames () {
  ...
  const styles = `.card--front { background-image: url("${desktop.games_solitaire.spriteImg}"); }`;
  css.appendChild(document.createTextNode(styles));

  document.head.appendChild(css);
  document.getElementById('js-deck-pile').onclick = desktop.games_solitaire.restartDeal;
  return true;
};

desktop.games_solitaire.openWindow = function openWindow () {
  desktop.games_solitaire.initSolitaire();
  return true;
};

desktop.games_solitaire.closeWindow = function closeWindow () {
  $(window).off('mousemove', desktop.games_solitaire.handleMove);
  $(window).off('mouseup', desktop.games_solitaire.releaseMove);

  const { state } = desktop.games_solitaire;
...
(function jsSolitaire() {
  ...
  const win = (canvasWidth, canvasHeight, canvasLeft, canvasTop) => {
    ...
    image.src = desktop.games_solitaire.spriteImg;
    ...
    const { winAnimationIntervals } = desktop.games_solitaire;
  ...
  desktop.games_solitaire.initSolitaire = initSolitaire;
  desktop.games_solitaire.state = state;
  desktop.games_solitaire.restartDeal = restartDeal;
  desktop.games_solitaire.gameFinish = gameFinish;
  desktop.games_solitaire.handleMove = handleMove;
  desktop.games_solitaire.releaseMove = releaseMove;
})();
