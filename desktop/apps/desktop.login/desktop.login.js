...
desktop.login.success = function desktopLoginSuccess() {
  ...
  clippy.load("Merlin", function (agent) {
    ...
    setTimeout(function () {
      if (!buddypond.email || buddypond.email.length < 3) {
        agent.speak(
          "Welcome to Buddy Pond! Please be sure to set your email address in the Profile App!"
        );
        setTimeout(function () {
          agent.hide();
        }, 12000);
        ...
        agent.speak(
          `Welcome back ${buddypond.me}! Merlin thinks you are fantastic!`
        );
        agent.animate();
        setTimeout(function () {
          agent.hide();
  ...
  setInterval(function () {
    ...
    $(".connection_average_response_time").html(
      buddypond.averageResponseTime()
    );
