...
desktop.app.buddylist.load = function desktopLoadBuddyList (params, next) {
  ...
  desktop.load.remoteAssets([
    ...
    function updatePositiveAffirmation () {
      const key = desktop.app.buddylist.positiveAffirmations[Math.floor(Math.random() * desktop.app.buddylist.positiveAffirmations.length)];
    ...
    $('.sendBuddyRequest').on('click', function () {
      ...
      const buddyName = $('.buddy_request_name').val() || $(this).html();
      if (!buddyName) {
    ...
    $('.inviteBuddy').on('click', function () {
      ...
      const inviteMessage = randomInviteMessages[Math.floor(Math.random() * randomInviteMessages.length)];
    ...
    d.on('mousedown', '.buddy_emoji_picker', function (ev) {
      const holder = $(ev.target).parent().parent();
      const textarea = $('.buddy_message_text', holder);
    ...
    d.on('mousedown', '.insertBuddySnap', function (ev) {
      const form = $(ev.target).parent();
      const context = $('.buddy_message_to', form).val();
    ...
    d.on('mousedown', '.insertBuddyGif', function (ev) {
      const form = $(ev.target).parent().parent();
      const output = 'buddy';
      const context = $('.buddy_message_to', form).val();
    ...
    d.on('mousedown', '.insertBuddySound', function (ev) {
      const form = $(ev.target).parent();
      const to = $('.buddy_message_to', form).val();
...
desktop.app.buddylist.renderChatWindow = function (context) {
  const clone = $('#window_buddy_message_0').html();
  const dockItemClone = $('#icon_dock_buddy_message_0').html();

  const window_id = 'window_buddy_message_' + context;
  ...
  const buddyChatStr = '<div id="' + window_id + '" class="abs window buddy_message"  data-app="buddylist" data-type="biddu_message" data-context="' + context + '">' + _clone + '</div>';
...
desktop.app.buddylist.sendMessage = function sendBuddyMessage (context) {
  ...
  buddypond.sendMessage(message.to, message.text, function (err, _data) {
...
function processIncomingBuddyRequests (data) {
  ...
  for (const buddy in data.buddyrequests) {
...
function openNewBuddyMessagesNow (data) {
  const buddylist = data.buddylist;
  Object.keys(buddylist).forEach(function (b) {
    const profile = buddylist[b];
    if (profile && profile.newMessages === true) {
      const buddyName = b.replace('buddies/', '');
...
function checkForIncomingVoiceCall (data) {
  ...
  Object.keys(buddylist).forEach(function (b) {
    ...
    if (profile && profile.isCalling === true) {
      ...
      desktop.app.videochat.startCall(false, buddyName, function (err, _re) {
  ...
  if (buddies) {
    if (buddies.length === 0) {
      ...
      buddies = buddies.sort(function (a, b) {
        const profileA = data.buddylist[a];
        const profileB = data.buddylist[b];
      ...
      buddies.forEach(function (buddyKey) {
        const buddy = buddyKey.replace('buddies/', '');
        const profile = data.buddylist[buddyKey];
      ...
      $('.messageBuddy').on('click', function () {
        ...
        const windowId = '#window_buddy_message_' + context;
        if (!isMobile) {
...
function renderBuddyRequests (data) {
  ...
  if (data.buddyrequests) {
    ...
    const totalIncomingBuddyRequests = $('.pendingIncomingBuddyRequests li').length;
    ...
    $('.approveBuddyRequest', '.pendingIncomingBuddyRequests').on('click', function () {
      $(this).parent().hide();
      const buddyName = $(this).attr('data-buddyname');
...
function checkForNewStreamers (data) {
  ...
  if (data.system && data.system.motm && data.system.motm.streaming) {
    ...
    const streamer = data.system.motm.streaming;
...
desktop.app.buddylist.updateBuddyList = function updateBuddyList () {
  ...
  if (!buddypond.qtokenid) {
    ...
    buddypond.getBuddyProfile(desktop.app.buddylist.profileState, function (err, data) {
      ...
      const serializedBuddyList = JSON.stringify(data);
...
desktop.app.buddylist.processMessages = function processMessagesBuddylist (data, cb) {
  ...
  data.messages.forEach(function (message) {
    ...
    const windowId = '#window_buddy_message_' + buddyKey;
    ...
    if (message.card && message.card.type === 'snaps') {
      ...
      const arr = message.card.snapURL.split('.');
    ...
    const currentlyDisplayedMessages = $('.chatMessage', windowId);
...
desktop.app.buddylist.openWindow = function (params) {
  ...
  if (params.context) {
    ...
    const windowId = '#window_buddy_message_' + params.context;
...
desktop.app.buddylist.closeWindow = function (params) {
  ...
  if (params.context) {
    const index = desktop.app.buddylist.subscribedBuddies.indexOf(params.context);
