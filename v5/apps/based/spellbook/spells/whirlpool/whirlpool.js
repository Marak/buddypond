export default async function whirlpool({
    ...
    html2canvas(targetElement, {
        ...
        // Animation loop for swirling whirlpool effect
        function animate() {
            const elapsed = Date.now() - startTime;
            const progress = elapsed / duration;

            if (elapsed < duration) {
                // Calculate swirl intensity (peaks at 50% progress)
                const intensityProgress = Math.sin(progress * Math.PI); // 0 to 1 to 0
                const swirlStrength = maxWaveAmplitude * intensityProgress * intensity;
                ...
                // Apply swirling whirlpool effect
                for (let y = 0; y < rect.height; y++) {
                    for (let x = 0; x < rect.width; x++) {
                        ...
                        const angle = Math.atan2(dy, dx) + swirlStrength * (1 / (distance + 50)) * intensityProgress; // Avoid division by zero

                        // Convert back to Cartesian coordinates
                        const srcX = Math.floor(centerX + distance * Math.cos(angle));
                        const srcY = Math.floor(centerY + distance * Math.sin(angle));
                ...
                // Draw the distorted image data back
