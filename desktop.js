...
const totalNewImages = imgs.length;
let totalNewLoadedImages = 0;
...
desktop._use = function _use (app, params) {
  ...
  */
  ...
  desktop.app[app].load(params, function (err, _re) {
...
desktop.messages.process = function processMessages (apps) {
  ...
  if (!buddypond.qtokenid) {
    ...
    //
    const subscribedBuddies = desktop.app.buddylist.subscribedBuddies;
    // const subscribedPonds = desktop.app.buddylist.subscribedPonds;
    ...
    buddypond.getMessages(params, function (err, data) {
      ...
      const appNameProcessMessagesList = [];
...
desktop.off = function desktopEventEmitterOff (eventName, eventLabel) {
  ...
  if (desktop._events[eventName] &&  desktop._events[eventName].length) {
    desktop._events[eventName] = desktop._events[eventName].filter(function (event) {
      if (event.eventLabel === eventLabel) {
        return false;
      }
      return true;
    });
...
// starts photo record
// const MAX_FRAMES_PER_SNAP = 10;
// eslint-disable-next-line no-redeclare
const DEFAULT_SNAP_TIMER = 777;
...
desktop.playSnaps = function playSnaps (opts) {
  ...
  const delay = $(el).data('delay') || opts.delay || DEFAULT_SNAP_TIMER;
  $(el).data('delay', delay);
...
desktop.smartlinks.replaceYoutubeLinks = function (el) {
  ...
  if (cleanText) {
    const searchYouTubeLongLink = cleanText.search('https://www.youtube.com/watch?');
    const searchYouTubeShortLink = cleanText.search('https://youtu.be/');
    ...
    if (searchYouTubeLongLink !== -1) {
      ...
      const youtubeId = cleanText.substr(searchYouTubeLongLink + 32, 11);
      let youtubeVideoTime = cleanText.substr(searchYouTubeLongLink + 43, 13).split(' ')[0];
      const isValidYoutubeId = desktop.utils.isValidYoutubeID(youtubeId);
      const isValidYoutubeTime = desktop.utils.isValidYoutubeTime(youtubeVideoTime);
      ...
      if (isValidYoutubeId) {
        ...
        */
        const str = 'https://www.youtube.com/watch?v=' + youtubeLinkWithTime;
    ...
    if (searchYouTubeShortLink !== -1) {
      ...
      */
      const youtubeId = cleanText.substr(searchYouTubeShortLink + 17, 11);
      ...
      if (isValidYoutubeId) {
        ...
        */
        const str = 'https://youtu.be/' + youtubeLinkWithTime;
        cleanText = cleanText.replace(str, `<a title="Open Youtube IDC" class="openIDC youtubeSmartLink" href="#open_IDC" data-videoid="${youtubeId}" data-videot="${youtubeVideoTime}"><img alt="Youtube" title="Youtube" src="desktop/assets/images/icons/youtube-logo.png"/>${youtubeId}@${youtubeVideoTime}</a>`);
        el.last().html(cleanText);
      }
    }
